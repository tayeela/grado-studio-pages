...
(function (root, factory) {
  ...
  // СИСТЕМАТИЧЕСКИМ сдвигом 7,34 м относительно ЕГРН (НСПД) и спутника.
  //
  ...
  //   • округ 77 (Москва):        dE=−4,819  dN=+5,540  (4081 пара, медиана
  //                               остатка 8 см, p90 12 см)
  //   • округ 50 (ТиНАО/область): dE=−4,817  dN=+5,566  (4918 пар, медиана
  //                               остатка 3,6 см, p90 4,5 см)
  //   • разница поправок между округами 2,7 см → ОДНОЙ константы достаточно.
  // Подгонка полного 2D-подобия даёт поворот 0,8″ и масштаб 0,9 ppm — оба
  // геодезически ничтожны, значит расхождение ЧИСТО поступательное.
  //
  // Это НЕ зона проекции (UTM 36/37) и не пересчёт в другую СК: репроекция
  // применяется к обоим источникам одинаково и НЕ меняет их взаимного
  // положения. И это не «сырой Бессель»: если бы портал отдавал широты-долготы
  // на датуме Бесселя-МСК-Москва, расхождение было бы 112 м, а оно 7,3 м —
  // то есть портал уже отдаёт ~WGS84, но по СВОЕЙ реализации перехода
  // МСК→WGS84 (старый бесселевский путь), а НСПД/спутник — по современной.
  // Лечится ОДНОЙ постоянной поправкой ко всем координатам ГИС ОГД.
  ...
  const GISOGD_DATUM_SHIFT_UTM37 = Object.freeze([-4.818, 5.554]);   // [восток, север], м
