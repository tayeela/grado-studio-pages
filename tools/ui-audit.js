...
(function (root) {
  ...
  function ревизия(корень) {
    ...
    for (const el of все) {
      ...
      // 4. лишняя горизонтальная прокрутка
      if (el.scrollWidth > el.clientWidth + ПОРОГ_ВЫЛЕТА && getComputedStyle(el).overflowX === "auto"
          && el.clientWidth > 60)
        добавить("каша", el, `прокрутка вбок: ${el.scrollWidth} при ширине ${el.clientWidth}`);
