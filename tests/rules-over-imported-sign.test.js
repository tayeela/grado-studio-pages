...
// ---------- style_manual: человек выбрал знак САМ — правило его не трогает ----------
// Регресс от исправления выше: перекрывая правилом ИМПОРТНЫЙ style_id, тот же
// код заодно перекрывал и style_id, который человек только что выбрал в
// «Знак из библиотеки» (bindObjectStyleSelect, app-tep.js). Выпадающий список
// продолжал бы показывать выбор человека, а на холсте рисовалось бы правило —
// расхождение панели свойств и картинки.
{
  layer.rules = [{ field: "ВИД", op: "=", value: "сквер", style_id: "user.green" }];
  const ручнойВыбор = { id: 5, style_id: "ogd.redline", style_manual: true, props: { ВИД: "сквер" } };
  assert.equal(styleOf(ручнойВыбор).stroke, "#e11",
    "объект со style_manual обязан рисоваться СВОИМ знаком, даже когда правило " +
    "слоя совпало по значению — человек выбрал знак сам, а не получил его импортом");
  const безФлага = { id: 6, style_id: "ogd.redline", props: { ВИД: "сквер" } };
  assert.equal(styleOf(безФлага).stroke, "#0a0",
    "без style_manual поведение прежнее: знак импорта правило перебивает");
}
