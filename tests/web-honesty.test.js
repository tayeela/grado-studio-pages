...
assert.ok(!html.includes("cv-empty"), "подсказка «Холст пуст» с чертежа тоже убрана");
...
// ---------- 4. про подложку ----------
// Подложка выключена намеренно (внешние тайлы), включается в панели «Подложка»;
// подсказка на холсте, которая её предлагала, убрана вместе со всей подсказкой.
assert.match(html, /id="basemap-show"/, "выключатель подложки обязан оставаться в интерфейсе");
assert.match(app, /on\("basemap-show", "change"/, "и работать");
