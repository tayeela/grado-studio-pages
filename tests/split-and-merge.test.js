...
// ---------- проводка в приложении ----------
{
  ...
  // Проверка на весь список была хрупкой: добавление ЛЮБОГО инструмента,
  // собирающего ломаную (выноска), роняло её, хотя разрезание не тронуто.
  // Спрашиваем то, что и хотели спросить, — что split здесь есть.
  assert.match(app, /const TOOL_GEOM = \{ \.\.\.GEOM_OF_TOOL,[^}]*split: "polyline"[^}]*\};/,
