...
// ---------- линия, точка, дуга, окружность ----------
{
  ...
  // Круг — ПЛОЩАДНАЯ фигура: он ложится в слой зоны, даёт площадь в ТЭП и в
  // таблице, заливается на холсте. Раньше здесь стояло «внутри окружности тела
  // нет» — модель, в которой круг считался кривой. Из-за неё круглую зону
  // нельзя было схватить за середину: на экране сплошная заливка, а клик не
  // выбирал ничего. Ожидание перевёрнуто вместе с моделью, а не подогнано.
  assert.ok(hits({ circle: { cx: 0, cy: 0, r: 50 } }, 20, 0, TOL), "внутри круга — тело, попадание");
  assert.ok(!hits({ circle: { cx: 0, cy: 0, r: 50 } }, 80, 0, TOL), "снаружи круга — мимо");
  assert.ok(hits({ arc: { cx: 0, cy: 0, r: 50 } }, 0, 49.7, TOL), "дуга — за обводку");
  assert.ok(!hits({ arc: { cx: 0, cy: 0, r: 50 } }, 0, 20, TOL),
    "а внутри дуги тела нет и быть не должно: дуга — кривая");
}
