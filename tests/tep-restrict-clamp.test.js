...
// Без библиотеки клиппинга (как здесь, в Node) ядро откатывается на суммарную
// площадь зон, поэтому предупреждение обязано назвать именно эту причину.
assert.match(warning.msg, /без объединения|перекрытие вычтено дважды/i,
  "предупреждение обязано объяснить причину завышенной площади ограничений");
