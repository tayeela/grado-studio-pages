...
const БЮДЖЕТ = {
  ...
  // 439 → 452: частичный успех выгрузки — список причин отказа и его показ.
  // Рост осознанный: раньше на этом месте молча терялось всё загруженное.
  "app-data.js openDataFetch": 452,
  ...
  "app-labels-place.js drawFeatureGeometry": 92,    // размер, точка, дуга, окружность, линии
  ...
  // 258 → 206 → 209: список, свотч и разметка вынесены. Дальше НЕ разносим
  // осознанно: единственный крупный кусок (renderEditor, 71 строка) меняет
  // переменную окружения sel, и вынос потребовал бы протаскивать изменяемое
  // состояние наружу — связности прибавится больше, чем уйдёт размера.
  // 209 при пороге 150 — превышение умеренное, храповик держит.
  ...
  // Три функции ниже (158–166 при пороге 150) НЕ разносим осознанно:
  // превышение 5–11%, а все вложенные куски в них — по 21–32 строки и тянут
  // локальное состояние окна. Протаскивать его наружу ради десятка строк
  // значит добавить связности больше, чем убрать размера. Храповик держит.
