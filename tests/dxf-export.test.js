...
// ---------- потеря оформления названа вслух ----------
{
  // Сквозной прогон показал: зона со штриховкой уходит в DXF одной полилинией.
  // Это не ошибка выгрузки — в DXF R12 нет ни HATCH, ни заливки по контуру, и
  // формат выбран сознательно ради читаемости где угодно. Но молчать нельзя:
  // человек откроет чертёж в AutoCAD, увидит пустые контуры вместо зон и решит,
  // что выгрузка сломалась. Сообщение обязано назвать ограничение и следующий шаг.
  const dxf = fs.readFileSync(path.join(root, "app-dxf.js"), "utf8");
  assert.match(dxf, /counts\.decor \+= 1/,
    "зоны с заливкой или штриховкой обязаны считаться отдельно");
  assert.match(dxf, /не переносятся[\s\S]{0,40}DXF R12/,
    "сообщение обязано прямо называть ограничение формата");
  assert.match(dxf, /штриховкой по контуру/,
