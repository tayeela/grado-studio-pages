...
function finishDrawing() {
  ...
  if (geom === "polygon" && pts) {
    ...
    // молчаливый отказ хуже отказа: человек ставит три точки и не понимает,
    // почему на холсте пусто
    if (a) addFeature(L.id, { arc: a });
    else { toast("Три точки почти на одной прямой — дуга не построена", "warn"); state.drawing = null; draw(); return; }
    ...
    else { toast(`Радиус ${r.toFixed(2)} м меньше полуметра — окружность не создана`, "warn"); state.drawing = null; draw(); return; }
  }
  state.drawing = null; state.typed = "";
  draw();
}
...
function arcFrom3Pts(p0, pm, p1) {
  ...
  // Порог |c| абсолютный, а сам c растёт как КВАДРАТ размера чертежа: три
  // почти коллинеарные точки, расставленные на сотни метров, дают c порядка
  // 1e-3 и проверку проходят — с радиусом в миллиарды метров. На экране такая
  // «дуга» неотличима от прямой, зато её габарит попадает в пространственный
  // индекс и в «показать всё»: масштаб улетает, поиск объектов по области
  // начинает перебирать всё подряд. Тысяча километров — заведомо не дуга
  // городского чертежа (вся Москва укладывается в полсотни), но и плавный
  // поворот дороги радиусом в сотню километров этот порог не заденет.
  if (!isFinite(r) || r > 1e6) return null;
